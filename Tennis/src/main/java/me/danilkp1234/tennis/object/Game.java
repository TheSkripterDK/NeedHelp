package me.danilkp1234.tennis.object;

import me.danilkp1234.tennis.RollbackHandler;
import me.danilkp1234.tennis.Tennis;
import me.danilkp1234.tennis.data.DataHandler;
import me.danilkp1234.tennis.tasks.GameCountdownTask;
import org.bukkit.*;
...
public class Game {
    ...
    public Game(String gameName) {
        ...
        try {
            ...
            Tennis.getInstance().getLogger().severe("Failed to load lobbyPoint with metadata " + fileConfiguration.getString("games." + gameName + ".lobbyPoint") + " for gameName: '" + gameName + "'. ExceptionType: " + ex);
        }
        ...
        for (String point : fileConfiguration.getStringList("games." + gameName + ".spawnPoints")) {
            ...
            try {
                ...
                Tennis.getInstance().getLogger().severe("Failed to load spawnPoint with metadata " + point + " for gameName: '" + gameName + "'. ExceptionType: " + ex);
            }
        }
        ...
        for (String item : fileConfiguration.getStringList("games." + gameName + ".normalItems")) {
            try {
                ...
                Tennis.getInstance().getLogger().severe(gameName + " tried to load normal item that doesn't exist: " + item);
            }
        }
        ...
        for (String item : fileConfiguration.getStringList("games." + gameName + ".rareItems")) {
            try {
                ...
                Tennis.getInstance().getLogger().severe(gameName + " tried to load rare item that doesn't exist: " + item);
            }
        }
    ...
    public boolean joinGame(GamePlayer gamePlayer) {
        ...
        if (isState(GameState.LOBBY) || isState(GameState.STARTING)) {
            ...
            Tennis.getInstance().setGame(gamePlayer.getPlayer(), this);
            return true;
        } else {
    ...
    public void startCountdown() {
        new GameCountdownTask(this).runTaskTimer(Tennis.getInstance(), 0, 20);
    }
    ...
    public void assignSpawnPositions() {
        ...
        for (GamePlayer gamePlayer : getPlayers()) {
            try {
                ...
                Tennis.getInstance().getLogger().severe("Not enough spawn points to satisfy game needs (Game is " + getDisplayName() + ")");
            }
        }
    }
